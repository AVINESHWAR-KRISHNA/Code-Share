...
log_dir = "LOGs"                      # Directory where log files are stored
retention_days = 30                   # Number of days to retain log files
rotation_when = "midnight"            # Rotation time ('midnight', 'H', 'D', etc.)
rotation_interval = 1                 # Interval for time rotation
backup_count = 30                     # Number of backup files to keep (based on days for TimedRotatingFileHandler)
...
def setup_logger(log_file_name=None, config_path='config/logger_settings.toml'):
    ...
    log_format = config.get('logger', {}).get('format', '%(asctime)s - %(name)s - %(levelname)s - %(module)s - %(funcName)s - %(message)s - PID: %(process)d - TID: %(thread)d')
    date_format = config.get('logger', {}).get('datefmt', '%Y-%m-%d %H:%M:%S')
    rotation_when = config.get('logger', {}).get('rotation_when', 'midnight')
    rotation_interval = config.get('logger', {}).get('rotation_interval', 1)
    backup_count = config.get('logger', {}).get('backup_count', 30)  # Number of days for timed rotation
    ...
    # Create a TimedRotatingFileHandler with time-based rotation
    try:
        file_handler = logging.handlers.TimedRotatingFileHandler(
            log_file_path, when=rotation_when, interval=rotation_interval, backupCount=backup_count
        )
        file_handler.setLevel(default_level_file)
        file_handler.setFormatter(logging.Formatter(log_format, date_format))
        logger.addHandler(file_handler)
...
# config/logger_settings.toml

[logger]
# Log level settings
default_level_file = "INFO"           # Log level for the file handler
default_level_console = "WARN"        # Log level for the console handler
...
rotation_when = "midnight"            # When to rotate the log files ('midnight', 'H', 'D', etc.)
rotation_interval = 1                 # Interval for rotation (1 means every day if 'midnight' is set)
backup_count = 30                     # Number of backup files to keep (log retention for 30 days)